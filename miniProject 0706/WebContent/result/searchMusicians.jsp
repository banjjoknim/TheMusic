...
<body>
	...
	if (searchBy == null && searchV == null) {
		...
		if (searchBy.equals("musicians")) {
			pstmt = conn.prepareStatement(
			"SELECT distinct nickname, user_image_file FROM musics,users where upper(nickname) like upper(?) and musics.user_id = users.user_id");
					...
					<ul class="nav_bar_small_menu">
						<div class="nav_bar_search">
							<select class="nav_bar_select" name="bySearch">
								...
								<option style="outline: none;" value="musicians" selected>Musicians</option>
	...
	<div id="container">
		<div id="content1">
			...
			<% }else{
				do{%>
					<ul>
				<li><a href="../users/anotherUserInfo.jsp?nickName=<%=rs.getString("nickName") %>"><div class="img-content1 img1">
						<img src="../files/users/<%=rs.getString("user_image_file")%>" style="border-radius: 75px;">
						<div class="darkness" style="border-radius: 75px;"></div></a>
					</div>

					<%-- <div class="img-with-text">
						<a style="font-size:1em; font-weight:600;"><%=rs.getString(16)%></a>
					</div> --%>
					<div class="img-with-text">
						<a style="color:gray;"><%=rs.getString("nickName") %></a>
					</div></li>
			</ul>
				<% }while (rs.next()); 
			}
		...
		<div id="left_sidebar">
			<ul>
				...
				<li><input type="radio" name="result" value="All_Categories" id="All_Categories"
					checked="checked"
					OnClick="location.href='searchAll.jsp?search_value=<%=searchV%>&bySearch=all'">
					<label for="All_Categories"> All Categories</label></li>
				<li><input type="radio" name="result" value="Title" id="Title"
					OnClick="location.href='searchTitle.jsp?search_value=<%=searchV%>&bySearch=title'">
					<label for="Title"> Title</label></li>
				<li><input type="radio" name="result" value="Musicians" id="Musicians"
					checked="checked"
					OnClick="location.href='searchMusicians.jsp?search_value=<%=searchV%>&bySearch=musicians'">
					<label for="Musicians"> Musicians</label></li>
				<li><input type="radio" name="result" value="Albums" id="Albums"
					OnClick="location.href='searchAlbums.jsp?search_value=<%=searchV%>&bySearch=albums'">
					<label for="Albums">Albums</label></li>
			</ul>
